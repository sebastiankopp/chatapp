...
public class CUDAdapter extends AbstractDBAdapter{
	...
	public Conversation addConv(List<User> users){
		int id;
		try {
			...
			pst.executeBatch();
			con.commit();
			return new Conversation(id, users);
		} catch (Exception sqle){
			sqle.printStackTrace();
			doRollback();
			return null;
	...
	public boolean userToConv (User u, Conversation c){
		try {
			PreparedStatement pst = con.prepareStatement(ADD_USR_TO_CONV);
			pst.setInt(1, c.getConvId());
			pst.setInt(2, u.getUserid());
			pst.executeUpdate();
			con.commit();
			return true;
